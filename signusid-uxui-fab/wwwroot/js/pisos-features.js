...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function canDeleteEstado(estado) {
    if (nonDeletableNames.includes(estado.name)) {
      return { allowed: false, message: `El piso <strong>${estado.name}</strong> no se puede eliminar.` };
    } else if (estado.assets >= 1) {
      return { allowed: false, message: `El piso <strong>${estado.name}</strong> tiene <strong>${estado.assets}</strong> activo/s asignado/s, no se puede eliminar.` };
    } else {
      return { allowed: true, message: `¿Está seguro que desea borrar el piso <strong>${estado.name}</strong>?` };
    }
  }
  ...
  deleteBatchBtn.addEventListener('click', function () {
    ...
    if (allCanBeDeleted) {
      ...
      deleteModalBody.innerHTML = `
                <p>¿Está seguro que desea borrar los siguientes pisos?</p>
                <ul>
      ...
      confirmDeleteBtn.setAttribute(
        'href',
        `${eliminarBatchPisosUrl}?ids=${estadosSeleccionados.map(estado => estado.id).join(',')}`
      );
    } else {
