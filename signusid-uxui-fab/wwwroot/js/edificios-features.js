...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('[data-bs-target="#deleteModal"]').forEach(button => {
    button.addEventListener('click', function () {
      const id = this.getAttribute('data-id-edificio');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function canDeleteEstado(estado) {
    if (nonDeletableNames.includes(estado.name)) {
      return { allowed: false, message: `El edificio <strong>${estado.name}</strong> no se puede eliminar.` };
    } else if (estado.assets >= 1) {
      return { allowed: false, message: `El edificio <strong>${estado.name}</strong> tiene <strong>${estado.assets}</strong> activo/s asignado/s, no se puede eliminar.` };
    } else {
      return { allowed: true, message: `¿Está seguro que desea borrar el edificio <strong>${estado.name}</strong>?` };
    }
  }
  ...
  deleteBatchBtn.addEventListener('click', function () {
    ...
    if (allCanBeDeleted) {
      ...
      deleteModalBody.innerHTML = `
                <p>¿Está seguro que desea borrar los siguientes edificios?</p>
                <ul>
      ...
      confirmDeleteBtn.setAttribute(
        'href',
        `${eliminarBatchEmpresaUrl}?ids=${estadosSeleccionados.map(estado => estado.id).join(',')}`
      );
    } else {
